...
if ($result = $mysqli->query($sqlSearch)) {
	...
	while($row = $result->fetch_assoc()) {
		...
		$corte=$row["corte"];
		$list[]=["id"=>"$id","cliente"=>"$cliente","apellido"=>"$apellido","ip"=>"$ip","fecha"=>"$today","email"=>"$mail","direccion"=>"$direccion","ciudad"=>"$ciudad","telefono"=>"$telefono","apuntamiento"=>"$apuntamiento","wallet"=>"$wallet","cedula"=>"$cedula","planPrice"=>"$planPrice","speed"=>"$speed","corte"=>"$corte"];
	}
}
